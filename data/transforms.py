...
class CustomSampleCropd(mt.Transform):
    def __init__(
        ...
        self.crop_content = mt.CropForegroundd(keys=self.keys, source_key="ct_vol", allow_smaller=True)
        ...
        self.foreground_crop = mt.RandCropByPosNegLabeld(
            ...
            pos=2.0, neg=1.0,
            num_samples=patch_per_sample,
            allow_smaller=True
        )

    def __call__(self, data: Dict[Hashable, torch.Tensor]) -> [Dict]:  # Dict[Hashable, torch.Tensor]:
...
def get_transforms(
    ...
    """
    input_keys = ["ct_vol", "pet_vol"]
    if stage in ["train", "val", "val2", "val_sampled"]:
        keys = ["ct_vol", "pet_vol", "mask"]
        out = ["image", "mask"]
    ...
    if use_cascading:
        input_keys.append("pred_mask")
        keys.append("pred_mask")
        mode += ("nearest",)
    ...
    shift_transforms = [
        mt.ScaleIntensityRanged(
            keys=["ct_vol"], a_min=ct_percentiles[0], a_max=ct_percentiles[1], b_min=0, b_max=1, clip=True
        ),
        mt.ScaleIntensityRanged(
            keys=["pet_vol"], a_min=pet_percentiles[0], a_max=pet_percentiles[1], b_min=0, b_max=1, clip=True
        ),
    ]
    ...
    if stage == "train":
        other_transforms = [
            ...
            CustomSampleCropd(
                keys=keys,
                label_key="mask",
                roi_size=tuple(int(math.sqrt(2) * x * 1.2) for x in target_shape),
                patch_per_sample=patch_per_sample
            ),
            mt.RandAffined(
                keys=keys,
                mode=mode,
            ...
            mt.RandCoarseDropoutd(
                ...
                fill_value=0,
                prob=0.8
            ) if use_cascading else mt.Identityd(keys=keys),
        ]
        ...
        other_transforms = [
            ...
            CustomSampleCropd(keys=keys, label_key="mask", roi_size=target_shape),
        ]
        all_transforms.extend(other_transforms)
    ...
    normalization_transforms = [
        ...
        mt.ScaleIntensityRanged(
            keys=["ct_vol"], a_min=0, a_max=1, b_min=ct_percentiles[0], b_max=ct_percentiles[1], clip=True
        ),
        mt.ScaleIntensityRanged(
            keys=["pet_vol"], a_min=0, a_max=1, b_min=pet_percentiles[0], b_max=pet_percentiles[1], clip=True
        ),
        mt.NormalizeIntensityd(keys=["ct_vol"], subtrahend=ct_norm[0], divisor=ct_norm[1]),
        mt.NormalizeIntensityd(keys=["pet_vol"], subtrahend=pet_norm[0], divisor=pet_norm[1]),
    ]
