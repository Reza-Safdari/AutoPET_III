...
# from .plans_handling.plans_handler import ConfigurationManager, PlansManager
...
class UNetResDecoder(nn.Module):
    ...
    def forward(self, skips):
        ...
        # else:
        #     target_shape = seg_outputs[0].shape[2:]
        #     seg_outputs = [
        #         interpolate(out, target_shape) if out.shape[2:] != target_shape else out for out in seg_outputs[:-1]
        #     ]
        #     return torch.stack(seg_outputs, dim=1)
        out = seg_outputs[0]
        if self.training and self.deep_supervision:
            target_shape = out.shape[2:]
            seg_outputs = [
                interpolate(out, target_shape) if out.shape[2:] != target_shape else out for out in seg_outputs[:-1]
            ]
            out = torch.stack(seg_outputs, dim=1)
        return out 
